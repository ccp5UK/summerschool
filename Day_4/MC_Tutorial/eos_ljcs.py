...
# They are NOT responsible for this Python code, which was written by Michael P Allen
# and any mistakes are the sole responsibility of MPA!
...
"""Equation of State for Lennard-Jones cut-and-shifted pair potential with rcut=2.5."""

# Key parameters for the EOS
...
def func ( rho, T ):
    """Returns P_diff and mu_diff for two densities rho at same temperature T, for rho_coex"""

    eos0 = eos ( T, rho[0] )
    eos1 = eos ( T, rho[1] )
    return eos0['P'] - eos1['P'], eos0['mu'] - eos1['mu']
    
def rho_coex ( T ):
    """Determines coexistence densities for given temperature according to EOS"""

    import numpy as np
    from scipy.optimize import root

    # Sanity check on T
    assert T>=0.64 and T<temp_crit, 'T out of range in rho_coex'

    # In solving the EOS equations for equal pressures and chemical potentials
    # we should avoid straying too far into the two-phase region,
    # since the EOS is not well behaved there.
    # So we need good initial estimates for the coexisting densities, and use the formula
    # due to J Vrabec, GK Kedia, G Fuchs, H Hasse, Mol Phys 104 1509 (2006)
    # (albeit they have a very slightly different value of temp_crit)
    TT = temp_crit - T
    rho_liq = rho_crit + 0.5649*TT**(1/3) + 0.1314*TT + 0.0413*TT**(3/2)
    rho_vap = rho_crit - 0.5649*TT**(1/3) + 0.2128*TT + 0.0702*TT**(3/2)

    # Now we solve, and this should work OK for T in the anticipated range
    sol = root ( func, np.array([rho_vap,rho_liq],dtype=np.float64), args=(T) )
    assert sol.success, 'Solution algorithm failed in rho_coex'
    rho_vap = sol.x[0]
    rho_liq = sol.x[1]
