...
int     Report			         ( void );
double 	f_equ			         ( int i, double ux, double uy, double density);
...
double	sig_xx   [XLENGTH]	[YLENGTH];                                         /* stress component                             */
double	sig_xy   [XLENGTH]	[YLENGTH];                                         /* stress component                             */
double	sig_yy   [XLENGTH]	[YLENGTH];                                         /* stress component                             */
double 	OMEGA = 1.00;

int main (int argc, char *argv[])
{	int	count, T;
	T = 0;
	// read command-line arguments and use first one after executable for number of timesteps
	if (argc>1) T = atoi(argv[1]);
	// if no valid command-line argument, set number of timesteps to default (4000)
	if (T==0) T = 4000;
...
void	Calc_stress	( void )
/* Calculates macroscopic stress */
{	int     x, y, i;
	double	f_neq;
	for (x = 0; x < XLENGTH; x++)
		for (y = 0; y < YLENGTH; y++)
		{	sig_xx[x][y]=sig_xy[x][y]=sig_yy[x][y]=0.0;
			for (i = 1; i < Q; i++)
			{	f_neq       = f[x][y][i] - f_equ (i,ux[x][y],uy[x][y],rho[x][y]);
				sig_xx[x][y] += f_neq * Cix[i] * Cix[i];
				sig_xy[x][y] += f_neq * Cix[i] * Ciy[i];
				sig_yy[x][y] += f_neq * Ciy[i] * Ciy[i];
                /* Following line prints non-equilibrium distribution functions to screen: commented out for progress bar
				printf("f^(1)(%d,%d,%d)=%f\n",x,y,i,f_neq);
                */
			}
			sig_xx[x][y] *= rho[x][y] / 3.0 / OMEGA;
			sig_xy[x][y] *= rho[x][y] / 3.0 / OMEGA;
			sig_yy[x][y] *= rho[x][y] / 3.0 / OMEGA;
		}
	return;
}
...
{	int     x, y, i;
	for (x = 0; x < XLENGTH; x++)
		for (y = 0; y < YLENGTH; y++)
			for (i = 0; i < Q; i++)	
			    f[x][y][i] += OMEGA * ( f_equ (i, ux[x][y], uy[x][y], rho[x][y]) - f[x][y][i] );			           
   return;
}
...
{	int	x, y;
	...
	fflush (f1); fclose (f1);
	f1 = fopen ("data_stress_xy.csv","w");
	for (y = 0; y < YLENGTH_SYS; y++)
	{	for (x = 0; x < XLENGTH; x++)
       		fprintf(f1,"%f,",sig_xy[x][y]);
       	fprintf(f1, "\n");
    }
	fflush (f1); fclose (f1);
...
{	double uDOTC, uDOTu;
	...
	uDOTu = (ux*ux+uy*uy);
	return (t[i]*density*(1.0+3.0*uDOTC-1.5*uDOTu+4.5*uDOTC*uDOTC));
}
