...
program mc_nvt
  ...
  real(dp), allocatable :: r_data(:,:,:) ! Configurations at each step (3,n,nr)

  integer  :: i, step, nstep, gap, nwidom, r_accepted, ir, nr, iw, nw, ioerr
  real(dp) :: vol, u, w, r_percent, cpu0, cpu
  logical  :: overlap

  character(*), parameter :: xyz_file  = 'mc_nvt.xyz'  ! Configuration file (overwritten at end)
  character(*), parameter :: hdf5_file = 'mc_nvt.hdf5' ! HDF5 file for simulation results
  ...
  call write_hdf5_dataset ( 'r', r_data   )
