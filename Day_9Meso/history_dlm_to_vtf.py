#!/usr/bin/env python3
"""Usage:
    history_dlm_to_vtf.py [--in <histin>] [--out <vtfout>] 
                          [--mscale <mscale>] [--lscale <lscale>] 
                          [--first <first>] [--last <last>] [--step <step>] 
                          [--separate]

Converts DL_MESO_DPD HISTORY file into VMD trajectory (VTF) file or structure 
(VSF) and coordinate (VCF) files

Options:
    --in <histin>       Name of DL_MESO_DPD-format HISTORY file to read in and
                        convert [default: HISTORY]
    --out <vtfout>      Name of VTF format file(s) to write 
                        [default: traject.vtf]
    --mscale <mscale>   DPD mass scale in daltons or unified atomic mass units 
                        [default: 1.0]
    --lscale <lscale>   DPD length scale in Angstroms [default: 1.0]
    --first <first>     Starting DL_MESO_DPD HISTORY file frame number for 
                        inclusion in VTF/VCF file [default: 1]
    --last <last>       Finishing DL_MESO_DPD HISTORY file frame number for 
                        inclusion in VTF/VCF file (value of 0 here will use 
                        last available frame) [default: 0]
    --step <step>       Incrementing number of frames in DL_MESO_DPD HISTORY 
                        between frames in VTF/VCF file [default: 1]
    --separate          Write separate files for structure (traject.vsf) and 
                        coordinates (traject.vcf), using user-specified name 
                        in place of 'traject' if required
